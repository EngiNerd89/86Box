...
void
cordata_vid_recalctimings(ogc_t *ogc)
{
...
void
cordata_vid_out(uint16_t addr, uint8_t val, void *priv)
{
...
uint8_t
cordata_vid_in(uint16_t addr, void *priv)
{
    ogc_t *ogc = (ogc_t *)priv;
...
void
cordata_vid_write(uint32_t addr, uint8_t val, void *priv)
{
    ogc_t *ogc = (ogc_t *)priv;
	int offset;
	...
	egawrites++;
    cordata_vid_waitstates(&ogc->cga);
}

uint8_t
cordata_vid_read(uint32_t addr, void *priv)
{
...
cordata_vid_poll(void *priv)
{
	...
	if (!(ogc->cga.cgamode & 0x80 || !(ogc->cga.cgamode & 2))) {
		/* standard cga mode */
		cga_poll(&ogc->cga);
		return;
	} else {
...
void
cordata_vid_speed_changed(void *priv)
{
    ogc_t *ogc = (ogc_t *)priv;

    cordata_vid_recalctimings(ogc);
}

void
cordata_vid_mdaattr_rebuild(){
	int c;
...
cordata_vid_init(ogc_t *ogc)
{
    ...
    loadfont_ex(L"roms/machines/cordata_pc400/100078-003.bin", L"roms/machines/cordata_pc400/100078-004.bin", 10, 0);
    ...
    timer_add(&ogc->cga.timer, cordata_vid_poll, ogc, 1);
    mem_mapping_add(&ogc->cga.mapping, 0xb8000, 0x08000,
		    cordata_vid_read, NULL, NULL,
		    cordata_vid_write, NULL, NULL,  NULL, 0, ogc);
    io_sethandler(0x03d0, 16, cordata_vid_in, NULL, NULL, cordata_vid_out, NULL, NULL, ogc);
...
const device_config_t cordata_vid_config[] =
{
        {
...
const device_t cordata_vid_device =
{
	"Olivetti OGC (GO708)",
	0, 0,
	NULL,
...
int
machine_xt_cpc400_init(const machine_t *model)
{		
    int ret;

    ret = bios_load_linear(L"roms/machines/cordata_pc400/Corona Data Systems PPC400 - System-ROM 4.23 CG.bin",
			   0x000fc000, 16384, 0);
    ...
    if (gfxcard == VID_INTERNAL) {
        ...
        vid = (ogc_t *)malloc(sizeof(ogc_t));
        memset(vid, 0x00, sizeof(ogc_t));
        
        cordata_vid_init(vid);

        device_add_ex(&cordata_vid_device, vid);
    }
    ...
    //bpp 1, width 2048, height 16
    //Corona PPC400 - 100078-003.bin even
    //Corona PPC400 - 100078-004.bin odd
    nmi_init();
